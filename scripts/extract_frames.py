...
def frames_output_path(video_name):
  return Path(f"data/processed/frames/{video_name}/.done")

def metadata_output_path(video_name):
  return Path(f"data/processed/frame_metadata/{video_name}_metadata.parquet")
...
def run_frame_extraction(fps, video_names, force=False):
  ...
  if video_names:
    todo = filter_missing(video_names, frames_output_path, force=force)
    report_skip("frame extraction", video_names, todo)
